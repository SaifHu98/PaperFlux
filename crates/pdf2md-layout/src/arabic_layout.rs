...
impl ArabicLayoutAnalyzer {
    ...
    pub fn classify_arabic_heading(text: &str) -> Option<(u8, f32)> {
        ...
        if trimmed.starts_with("المبحث ") || trimmed.starts_with("المادة ") || trimmed.starts_with("مادة ") {
            return Some((3, 0.94));
        }
        if trimmed.starts_with("المطلب ") || trimmed.starts_with("الفرع ") || trimmed.starts_with("أولاً:") || trimmed.starts_with("ثانياً:") {
            return Some((4, 0.92));
        }
    ...
    pub fn detect_arabic_list_item(text: &str, indent_pt: f32) -> Option<(bool, Option<String>, String, usize)> {
        ...
        let arabic_abjad = ['أ', 'ب', 'ج', 'د', 'ه', 'و', 'ز', 'ح', 'ط', 'ي', 'ك', 'ل', 'م', 'ن', 'س', 'ع', 'ف', 'ص', 'ق', 'ر', 'ش', 'ت', 'ث', 'خ', 'ذ', 'ض', 'ظ', 'غ'];
        ...
        if let Some(pos) = trimmed.find(|c| c == '.' || c == ')' || c == ']') {
            let prefix = trimmed[..pos].trim_start_matches('(').trim_start_matches('[').trim();
    ...
    pub fn detect_arabic_caption(spans: &[TextSpan]) -> Option<Node> {
        ...
        let target_type = if trimmed.starts_with("شكل ") || trimmed.starts_with("الشكل ") || trimmed.starts_with("صورة ") {
            CaptionTarget::Figure
        } else if trimmed.starts_with("جدول ") || trimmed.starts_with("الجدول ") {
            CaptionTarget::Table
        } else if trimmed.starts_with("مخطط ") || trimmed.starts_with("كود ") || trimmed.starts_with("خوارزمية ") {
            CaptionTarget::Code
