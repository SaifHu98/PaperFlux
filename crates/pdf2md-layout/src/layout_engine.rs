...
impl LayoutEngine {
    ...
    pub fn analyze_page(&self, page: &RawPage) -> Section {
        ...
        let footnote_detector =
            FootnoteDetector::new(page.height, heading_classifier.base_body_font_size);

        // 4. Staged Reading Order decomposition (handling multi-column, sidebars, full-width headers)
        let layout_blocks = self
            .reading_order_engine
            .compute_reading_order(&content_spans, page.width);
